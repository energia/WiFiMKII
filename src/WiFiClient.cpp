...
int WiFiClient::sslConnect(IPAddress ip, uint16_t port)
{
    ...
    if ( iRet < 0 && (iRet != SL_ERROR_BSD_ESECSNOVERIFY && iRet != SL_ERROR_BSD_ESECDATEERROR) ) {
        sslLastError = iRet;
        sl_Close(socketHandle);
        return false;
    }
    ...
    if ( sslVerifyStrict && (iRet == SL_ERROR_BSD_ESECSNOVERIFY || iRet == SL_ERROR_BSD_ESECDATEERROR) ) {
        sslLastError = iRet;
        sl_Close(socketHandle);
        return false;
    }

    if (iRet == SL_ERROR_BSD_ESECSNOVERIFY || iRet == SL_ERROR_BSD_ESECDATEERROR) {
        sslLastError = iRet;
        sslIsVerified = false;
    }
...
int WiFiClient::available()
{
    ...
    if (bytesLeft <= 0) {
        ...
        SL_SOCKET_FD_ZERO(&errorsds);
        SL_SOCKET_FD_SET(WiFiClass::_handleArray[_socketIndex], &readsds);
        SL_SOCKET_FD_SET(WiFiClass::_handleArray[_socketIndex], &errorsds);
...
//        if(!SL_FD_ISSET(WiFiClass::_handleArray[_socketIndex], &readsds)) return 0;
        ...
        if ((iRet <= 0)  &&  (iRet != SL_ERROR_BSD_EAGAIN)) {
        ...
        //(if SL_ERROR_BSD_EAGAIN was received, the actual number of bytes received was zero, not -11)
        //
        rx_currentIndex = 0;
        rx_fillLevel = (iRet != SL_ERROR_BSD_EAGAIN) ? iRet : 0;
